...
def read_data0():
    ...
    del df
    res = pd.read_csv('file0.txt', index_col=[0] , header=None,
...
def Distance(lng1, lat1, lng2, lat2):
    ...
    s = np.arccos(np.sin(radLat1) * np.sin(radLat2) +
...
def Liner_intrp(df, i):
    ...
    t['id'] = t['id'].fillna(i)
    t['longitude'] = t['longitude'].interpolate(method='linear', limit_direction='forward', axis=0)
    t['latitude'] = t['latitude'].interpolate(method='linear', limit_direction='forward', axis=0)
...
if not os.path.exists('file_pre.txt'):
    if not os.path.exists('file.txt'):        
        if os.path.exists('file0.txt'):
            df = pd.read_csv('file0.txt', index_col=[0] , header=None,
                                names=['id', 'time', 'longitude', 'latitude'],
                                parse_dates=['time'], engine='python')
        else:
            df = read_data0()
    ...
    df = pd.read_csv('file.txt', index_col=[0], parse_dates=['time'],
...
#             df = pd.read_csv('sub_file0.txt', index_col=[0] , header=None,
#                                 names=['id', 'time', 'longitude', 'latitude'],
#                                 parse_dates=['time'], engine='python')
#         else:
#             df = read_data0()
...
#     df = pd.read_csv('sub_file.txt', index_col=[0], parse_dates=['time'],
#                         usecols=[0, 1, 2, 3], engine='python')
#     df = interpolation(df)
